...
module.exports.config = {
  ...
  version: "2.2.0",
  hasPermssion: 0,
  credits: "RX api x MOHAMMAD AKASH ",
  description: "Download audio from YouTube",
  commandCategory: "media",
  usages: "sing [song name | YouTube link]\nExample: sing chipi chipi chapa chapa",
  cooldowns: 5
};
...
module.exports.handleReply = async function ({ api, event, handleReply }) {
  ...
  try {
    ...
    api.sendMessage("Error downloading audio (may exceed 26MB).", event.threadID);
  }
};
...
module.exports.run = async function ({ api, event, args }) {
  ...
  if (isYtLink) {
    ...
    try {
      ...
      console.error(err);
      return api.sendMessage("Failed to fetch audio from link.", threadID, messageID);
    }
  }
  ...
  try {
    ...
    let msg = "Choose a song (reply with number 1–6):\n\n";
    const thumbs = [];
    ...
    api.sendMessage("Error searching for songs.", threadID, messageID);
  }
};
